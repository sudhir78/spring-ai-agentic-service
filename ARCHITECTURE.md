...
graph TD
    ...
    style S fill:#FF9800,stroke:#EF6C00,color:white
...
### 3. Service Layer
- **ChatService**: Contains business logic for chat functionality
  - Uses Spring AI's `ChatClient` for AI interactions
  - Processes user messages and returns AI responses
  - Implements the core chat logic
...
3. **Service Processing**: `ChatService` passes message to `ChatClient` for AI processing
4. **AI Integration**: `ChatClient` constructs prompt and calls OpenAI API
...
- `spring.ai.openai.chat.options.model`: Specifies GPT model (gpt-4.1-mini)
